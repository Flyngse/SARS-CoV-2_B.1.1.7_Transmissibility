...
data parms_3 ;
	set parms_2 ;
		*where age_index <= 80 
		...
		;
		*if pos_sec <5 then pos_sec_text = "<5" ;
		...
		/*
		n = '(Potential secondary cases = ' || strip(put(count_pot_sec,comma20.)) || ')' ;
		q = '(Primary cases = ' || strip(put(count_index,comma20.)) || ')' ;
		z = '(Primary cases = ' || strip(put(count_index,comma20.)) || ')' ;
		*/
run;

/*
ods listing gpath = "F:\Projekter\FSEID00004942\FrederikPlesnerLyngse\Household Transmission\WGS\Output Revision 1\" ;
ods graphics / reset=index imagename = "Age_by_Age_TransmissionRATE" imagefmt=pdf ;
...
proc sgplot data = parms  noborder noautolegend ;
	...
	text 
				x = age_index
				y = age_secondary
				text = standarderror / position=bottom textattrs=(size=9px)
				;
...
proc sgplot data = parms_3  noborder noautolegend ;
	*by  lineage ;
	...
	text 
				x = age_index
				y = age_secondary
				text = estimate2 / position=top vcenter=baseline
				;
	text 
				x = age_index
				y = age_secondary
				text = q / position=bottom textattrs=(size=9px)  splitchar=";" splitpolicy=splitalways vcenter=baseline 
				;
